	...
	stages {
		...
		stage('Code Analysis') {
		    ...
		    script{
			    ...
			    sh "$WORKSPACE/cpilint/cpilint-1.0.5/bin/cpilint -rules $WORKSPACE/rules.xml -files $WORKSPACE/IntegrationContent/IntegrationArtefacts/ZipFiles/${env.Filename}"
		    }
	    }
        }
    }
